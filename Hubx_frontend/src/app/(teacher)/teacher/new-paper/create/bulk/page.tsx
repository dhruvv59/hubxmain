...
function BulkPageContent() {
    ...
    const handleFileUpload = async (file: File) => {
        ...
        try {
            ...
            if (successCount > 0) {
                showToast(`File processed: ${successCount} questions added!${failCount > 0 ? ` (${failCount} failed)` : ""}`, "success");
            } else if (failCount > 0) {
                showToast(`${failCount} questions failed to upload. Please check your file format.`, "warning");
            }

        } catch (error: any) {
            console.error("Bulk upload failed:", error);
            showToast(error?.message || "Upload failed. Please check file format and try again.", "error");
        } finally {
