...
async function runActivitiesTest() {
    ...
    const stdRes = await post('/teacher/standards', { name: "Class " + Date.now() }, teacherToken);
    ...
    // Correct payload for question
    const qRes = await post(`/teacher/papers/${paperId}/questions`, {
        questionText: "What is 2 + 2?", // Fixed key
        type: "MCQ",
    ...
    if (qRes.status === 201) {
        ...
        // Try falling back to multipart if needed? No, JSON should work if we fixed the key.
    }
